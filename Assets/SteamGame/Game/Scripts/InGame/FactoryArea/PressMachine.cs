...
namespace TechC
{
    public class PressMachine : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject pressObj, pipeObj;
        ...
        private Vector3 pressInitPos, pipeInitPos;
        private float elapsedTime = 0;

        private void Awake()
        {
            pressInitPos = pressObj.transform.position; // 初期位置を取得
            pipeInitPos = pipeObj.transform.position;
            elapsedTime = 0;
        }
        ...
        private IEnumerator Press()
        {
            ...
            while (Vector3.Distance(pressObj.transform.position, pressTargetPos.position) > proximityThreshold)
            {
                ...
                // pipeObjの位置を同期
                pipeObj.transform.position = pipeInitPos + (pressObj.transform.position - pressInitPos);
